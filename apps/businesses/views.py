...
def billing_history(request):
    ...
    try:
        ...
        invoices = SubscriptionInvoice.objects.filter(
            subscription__business=request.tenant
        ...
        return redirect('businesses:subscription_overview')
...
def usage_analytics(request):
    ...
    try:
        from apps.subscriptions.models import Subscription, UsageMetric
        ...
        usage_summary = {
            'orders': {
                'used': sum(metric.orders_count for metric in usage_metrics),
                'limit': subscription.plan.max_orders if subscription else 1000,
            },
            'customers': {
                'used': sum(metric.customers_count for metric in usage_metrics),
                'limit': subscription.plan.max_customers if subscription else 500,
            },
            'storage': {
                'used': sum(metric.storage_mb for metric in usage_metrics),
                'limit': subscription.plan.max_storage_mb if subscription else 1024,
            },
            'api_calls': {
                'used': sum(metric.api_calls for metric in usage_metrics),
                'limit': subscription.plan.max_api_calls if subscription else 10000,
            },
        }
...
def subscription_settings(request):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            return redirect('businesses:subscription_settings')
